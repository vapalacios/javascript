...
function enfermeria(recinto, nombreRecinto, animal, enfermedad){
	return {
		recinto: recinto,
...
function modificarSaludAleatoria(recinto, index){
    ...
    for(var i = 0; i < recinto.animales.length; i++){
    	if(healthAleatorio()){
            if(animal[i].salud <= 80){
	           animal[i].salud = animal[i].salud + 20;
            }
	   }else{
	     animal[i].salud = animal[i].salud - 20;
	     if(animal[i].salud <= 50){
	     	visitarEnfermeria(animal.splice(i, 1), recinto.nombre, index);
	     }
	   }
    }
}
...
function visitarEnfermeria(recintoAnimal, nombreRecinto, index){
	var enfermo = enfermeria(index, rnombreRecinto, recintoAnimal, generarEnfermedadAleatorio());
	zoo.enfermeria.push(enfermo);
}
...
function recoveryHealth(){
	var recovery = zoo.enfermeria;
	for(var i in recovery){
		if(recovery[i].animal.salud < 100){
			recovery[i].animal.salud = recovery[i].animal.salud + 10;
		}
	}
}
